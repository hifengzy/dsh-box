...
一般是**残留的 dsh 服务进程占着端口**:App 被强制退出/崩溃时,dsh 子进程可能变孤儿继续存活,下次启动时新进程绑定端口失败(EADDRINUSE),而健康检查命中残留服务,造成误报。App 启动时会自动清理这类残留(按"本 App 的 dsh 入口 + 端口"签名匹配,不影响其它进程);若仍出现,可手动 `pkill -f "bin.js web --port 3260"` 后重开。
