...
- **顶栏视图** (`src/renderer/topbar.*`) 是独立的自定义标题栏:整条可拖动、双击最大化,左侧留红绿灯空间且**不显示应用名**;右侧 `.actions` 是功能入口区,当前放 **GitHub 仓库按钮**(50×50 无背景,悬停显示背景,图标 36×36 随 light/dark 变色,点击在系统浏览器打开 `https://github.com/hifengzy/dsh-box`,纯 HTML 想加就加)。
