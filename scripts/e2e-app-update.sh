...
#
# 流程:
#   1) package.json version 临时改 0.2.0 → electron-builder 构建 zip + latest-mac.yml(fake repo)
#   2) 还原 version 0.1.0 → electron-builder --mac dir 构建旧版 app(含 DSH_E2E_APP_UPDATE 钩子)
...
#
# 需要: 可写 ~/Library/Caches(electron-updater/Squirrel 硬编码)+ 构建(几 GB node_modules)。
set -euo pipefail
...
for i in $(seq 1 150); do
  ...
  if [ "$v" = "$NEW_VER" ] && [ "${procs:-0}" -ge 1 ]; then
    ...
    errs="$(grep -c "APP_UPDATE_ERROR" "$E2E_DIR/run.log" 2>/dev/null || echo 0)"
    ...
    echo "PASS ✓ 完整流程: 0.1.0 → 查/下载/安装 → Squirrel 替换 → 重启到 $NEW_VER"
    exit 0
  fi
  sleep 2
done
