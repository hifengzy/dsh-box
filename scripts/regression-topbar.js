 ...
 *   2. 右侧功能入口区有 GitHub 按钮:50×50、图标 36×36 居中、默认透明背景,
...
app.whenReady().then(async () => {
  try {
    ...
    if (dom.btnW !== "50px" || dom.btnH !== "50px")
      throw new Error(`按钮应为 50×50,实际 ${dom.btnW}×${dom.btnH}`);
    if (dom.iconW !== "36px" || dom.iconH !== "36px")
      throw new Error(`图标应为 36×36,实际 ${dom.iconW}×${dom.iconH}`);
    if (dom.btnBg !== "rgba(0, 0, 0, 0)")
      throw new Error(`按钮默认应为透明背景,实际 ${dom.btnBg}`);
    console.log(`[2] GitHub 按钮 50×50 / 图标 36×36 / 默认透明背景 ✓`);
